...
/**
 * The earliest start a link allows its successor, given where the predecessor
 * sits now. Used to tell a link that is being honoured from one the plan has
 * drifted past — the board draws the second in red.
 */
export function earliestStartFor(
  link: TimelineDependency,
  ...
  const durationDays = Math.max(
    ...
    differenceInCalendarDays(
      parseISO(successor.end_date),
      parseISO(successor.start_date),
    ),
  );
  switch (link.dep_type) {
    case "FS":
      return format(
        addDays(parseISO(predecessor.end_date), 1 + link.lag_days),
        "yyyy-MM-dd",
      );
    case "SS":
      return format(
        addDays(parseISO(predecessor.start_date), link.lag_days),
        "yyyy-MM-dd",
      );
    case "FF":
      return format(
        addDays(parseISO(predecessor.end_date), link.lag_days - durationDays),
        "yyyy-MM-dd",
      );
    case "SF":
      return format(
        addDays(parseISO(predecessor.start_date), link.lag_days - durationDays),
        "yyyy-MM-dd",
      );
  }
}

/** A link the plan no longer honours: the successor starts too early. */
export function isViolated(
  link: TimelineDependency,
  predecessor: TimelineItem,
  successor: TimelineItem,
): boolean {
  return successor.start_date < earliestStartFor(link, predecessor, successor);
}
