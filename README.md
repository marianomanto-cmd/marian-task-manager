...
- **Timeliner** (`/timeliner`): creador de timelines/Gantt para el equipo. Varios timelines (pestañas), tareas con duración, hitos, owners (lista del equipo), arrastrar para mover/estirar barras, fines de semana y feriados por país (AR/PA/US/ES) con toggle on/off, y export a Excel (.xlsx con grilla coloreada). Visible y editable por cualquier miembro logueado. Cada timeline tiene además su **link para el cliente** (ver abajo).
  ...
  - **MASTER**: pestaña que junta **sólo los hitos** de todos los timelines en un mismo calendario. Tiene su propio link compartible (ver abajo). Dos vistas: **Calendario**, una fila por proyecto sobre bandas de mes y la línea de hoy — cuando los hitos se amontonan la fila crece en sub-filas para que **todos** los títulos entren con su fecha, en vez de dejar rombos sin nombre; y **Lista**, los mismos hitos en orden cronológico agrupados por mes, con fecha, proyecto y owner. Zoom Detalle / Normal / Panorama (en Panorama quedan sólo los rombos, como mapa de densidad). Arranca centrado en hoy. Es de lectura: al tocar un proyecto o un hito te lleva a su timeline.
- **Link del MASTER** (`/m/<token>`): el MASTER completo en un link de sólo lectura y sin login, que se actualiza solo igual que los links por timeline. **Es para el equipo, no para un cliente**: muestra todos los proyectos con sus nombres y fechas, así que quien lo abre ve también los de los demás clientes — el equivalente de `/todos` en Proyectos. Sale del botón "Compartir" dentro de la pestaña MASTER, con la advertencia a la vista; ahí mismo se rota o se revoca. Hay un solo link de MASTER para todo el espacio. Sólo viajan los hitos: las tareas no salen por este link.
...
>
> Timeliner requiere aplicar `supabase/migrations/0024_timeliner.sql` (usa la tabla `holidays` ya existente); los links de timeline, `0029_timeline_public_links.sql`; el link del MASTER, `0031_timeline_master_share.sql`.
>
