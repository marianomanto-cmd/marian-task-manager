...
- **Timeliner** (`/timeliner`): creador de timelines/Gantt para el equipo. Varios timelines (pestañas), tareas con duración, hitos, owners (lista del equipo), arrastrar para mover/estirar barras, fines de semana y feriados por país (AR/PA/US/ES) con toggle on/off, y export a Excel (.xlsx con grilla coloreada). Visible y editable por cualquier miembro logueado. Cada timeline tiene además su **link para el cliente** (ver abajo).
  - **Proyecto base**: un timeline nuevo arranca con la plantilla ya cargada — las 22 tareas e hitos de una campaña estándar, en sus dos tracks (Creatividades y Plan de medios) como grupos. Elegís desde qué día arranca (por defecto el lunes que viene) y las fechas se acomodan solas manteniendo el día de semana de cada ítem. Se puede apagar con el switch "Con el proyecto base" al crear.
  - **MASTER**: pestaña que junta **todos los timelines** en un mismo calendario — tareas como barras e hitos como rombos, una fila por proyecto. Tiene su propio link compartible (ver abajo). Dos filtros arriba: **Todo / Sólo hitos / Sólo tareas**, y **Grupos**, un desplegable con los grupos de cada proyecto por separado (más "Sin grupo" donde hay elementos sueltos) para tildar uno o varios; un proyecto sin nada tildado muestra todo, así acotar una campaña no vacía las demás. Dos vistas: **Calendario**, sobre bandas de mes y la línea de hoy — cuando el trabajo se amontona la fila crece en sub-filas para que **todos** los títulos entren, en vez de dejar barras sin nombre, y el nombre del proyecto acompaña el scroll; y **Lista**, lo mismo en orden cronológico agrupado por mes, con fechas, proyecto y owner. Zoom Detalle / Normal / Panorama (en Panorama se apagan los títulos y queda el mapa de densidad). Arranca centrado en hoy. Es de lectura: al tocar un proyecto o un elemento te lleva a su timeline.
- **Link del MASTER** (`/m/<token>`): el MASTER completo en un link de sólo lectura y sin login, que se actualiza solo igual que los links por timeline. **Es para el equipo, no para un cliente**: muestra todos los proyectos con sus nombres y fechas, así que quien lo abre ve también los de los demás clientes — el equivalente de `/todos` en Proyectos. Sale del botón "Compartir" dentro de la pestaña MASTER, con la advertencia a la vista; ahí mismo se rota o se revoca. Hay un solo link de MASTER para todo el espacio. Trae lo mismo que la pestaña —tareas e hitos— con sus filtros incluidos.
...
>
> Timeliner requiere aplicar `supabase/migrations/0024_timeliner.sql` (usa la tabla `holidays` ya existente); los links de timeline, `0029_timeline_public_links.sql`; el link del MASTER, `0031_timeline_master_share.sql` y `0032_master_share_full.sql`.
>
