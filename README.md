...
- **Dependencias sin modal, arrastrando o con dos clicks**: al pasar el mouse por una barra aparecen dos puntitos, uno en cada punta. Se tira de uno hasta otra tarea y quedan vinculadas — o, si preferís, click en el puntito de origen y click en el de destino. La punta de la que salís y la que tocás definen el tipo (`FS` fin→inicio, `SS` inicio→inicio, `FF` fin→fin, `SF` inicio→fin).
...
5. **Los vínculos se crean con dos clicks, no arrastrando** — es todo lo que SVAR trae. El arrastre lo agrega `svar-gantt-inner.tsx` por encima (pointerdown en la punta, pointerup sobre la otra barra, `api.exec("add-link")`), sin pisar el flujo de clicks. Ojo también con `wx-weekend`: no es sólo un nombre, es la clase que estira la columna teñida a lo alto del chart; sin ella sólo se pinta la regla de fechas.
