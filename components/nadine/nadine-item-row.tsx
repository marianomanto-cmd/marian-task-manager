...
import {
  archiveNadineItemAction,
  deleteNadineItemAction,
  updateNadineItemAction,
...
import {
  ...
  NADINE_ITEM_CATEGORY_DOT,
  NADINE_ITEM_CATEGORY_LABEL,
  NADINE_ITEM_STATUSES,
  NADINE_ITEM_STATUS_BAR,
  NADINE_ITEM_STATUS_CLASS,
...
export function NadineItemRow({
  ...
  const updateMutation = useMutation({
    mutationFn: async (patch: Partial<NadineItem>) => {
      const result = await updateNadineItemAction({ id: item.id, ...patch });
  ...
  const archiveMutation = useMutation({
    mutationFn: async (archive: boolean) => {
      const result = await archiveNadineItemAction(item.id, archive);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: NADINE_ITEMS_KEY }),
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const result = await deleteNadineItemAction(item.id);
      if (!result.ok) throw new Error(result.message);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: NADINE_ITEMS_KEY }),
    onError: (err: Error) => showToast({ title: err.message }),
    ...
    >
      <span
        ...
        className={cn(
          ...
          NADINE_ITEM_STATUS_BAR[item.status],
        )}
      />
...
function CategoryCell({
  ...
  value: NadineItemCategory;
  canEdit: boolean;
  onChange: (next: NadineItemCategory) => void;
}) {
  const content = (
    <>
      <span
        className={cn("size-2.5 rounded-full", NADINE_ITEM_CATEGORY_DOT[value])}
      />
      <span className="text-muted-foreground text-xs md:hidden">
        {NADINE_ITEM_CATEGORY_LABEL[value]}
      </span>
    </>
  );
  ...
  if (!canEdit)
    return (
      <span
        ...
        title={NADINE_ITEM_CATEGORY_LABEL[value]}
      >
        {content}
      </span>
    );
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="flex h-8 items-center gap-1.5 md:h-6 md:justify-center"
          title={NADINE_ITEM_CATEGORY_LABEL[value]}
          aria-label={`Categoría: ${NADINE_ITEM_CATEGORY_LABEL[value]}`}
        >
          {content}
        </button>
      </DropdownMenuTrigger>
...
function StatusCell({
  ...
  value: NadineItemStatus;
  canEdit: boolean;
  onChange: (next: NadineItemStatus) => void;
}) {
  const chip = (
    <span
      className={cn(
        "inline-flex h-8 items-center gap-1.5 rounded-full border px-2.5 text-[11px] font-medium md:h-6 md:px-2",
        NADINE_ITEM_STATUS_CLASS[value],
      )}
    >
      <span
        className={cn("size-1.5 rounded-full", NADINE_ITEM_STATUS_DOT[value])}
      />
      {NADINE_ITEM_STATUS_LABEL[value]}
    </span>
  );
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start">
        {NADINE_ITEM_STATUSES.map((s) => (
          <DropdownMenuItem key={s} onClick={() => onChange(s)}>
            <span
              className={cn("mr-2 size-2 rounded-full", NADINE_ITEM_STATUS_DOT[s])}
            />
            {NADINE_ITEM_STATUS_LABEL[s]}
