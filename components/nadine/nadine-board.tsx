...
import {
  createNadineGroupAction,
  createNadineProjectAction,
  deleteNadineGroupAction,
  reorderNadineItemsAction,
} from "@/app/actions/nadine";
import { NadineGroupSection } from "@/components/nadine/nadine-group-section";
import {
  NADINE_ITEMS_KEY,
  nadineBoardKey,
  useNadineBoard,
} from "@/components/nadine/use-nadine-board";
...
const STATUS_OPTIONS = NADINE_ITEM_STATUSES.map((s) => ({
  value: s,
  label: NADINE_ITEM_STATUS_LABEL[s],
}));
const CATEGORY_OPTIONS = NADINE_ITEM_CATEGORIES.map((c) => ({
  value: c,
  label: NADINE_ITEM_CATEGORY_LABEL[c],
}));
...
export function NadineBoard() {
  const canEdit = true;
  const [view, setView] = React.useState<View>("active");
  const query = useNadineBoard({ archiveMode: view });
  const qc = useQueryClient();

  const [statuses, setStatuses] = React.useState<NadineItemStatus[]>([
    ...NADINE_ITEM_STATUSES,
  ]);
  const [categories, setCategories] = React.useState<NadineItemCategory[]>([
    ...NADINE_ITEM_CATEGORIES,
  ]);
  const [selectedGroup, setSelectedGroup] = React.useState<string | null>(null);
  const [search, setSearch] = React.useState("");
  const [showFilters, setShowFilters] = React.useState(true);
  const density: NadineDensityMode = "comfortable";
  ...
  const metaByProject = React.useMemo(() => {
    const map = new Map<string, NadineProjectMeta>();
  ...
  const grouped = React.useMemo(() => {
    const map = new Map<string, NadineItem[]>();
  ...
  const groupSections = React.useMemo(() => {
    const byGroup = new Map<string, { project: string; items: NadineItem[] }[]>();
  ...
  const reorderItemsMutation = useMutation({
    mutationFn: async (vars: { project: string; ids: string[] }) => {
      const result = await reorderNadineItemsAction(vars);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: nadineBoardKey({ archiveMode: view }) }),
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `board-nadine-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  const filtersActive =
    ...
    statuses.length !== NADINE_ITEM_STATUSES.length ||
    categories.length !== NADINE_ITEM_CATEGORIES.length;
    ...
    <section className="mx-auto flex w-full max-w-[80rem] flex-col gap-4 px-3 py-4 md:px-6 md:py-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl font-semibold tracking-tight md:text-2xl">
            {inArchive ? "Archivo · Board - Nadine" : "Board - Nadine"}
          </h1>
      ...
      {showFilters ? (
        <div className="flex flex-col gap-2">
          <FilterChips<NadineItemStatus>
            label="Status"
            options={STATUS_OPTIONS}
            selected={statuses}
            onChange={setStatuses}
          />
          <FilterChips<NadineItemCategory>
            label="Categoría"
...
function ManageGroups({
  ...
  const createMutation = useMutation({
    mutationFn: async (n: string) => {
      const result = await createNadineGroupAction(n);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (n: string) => {
      const result = await deleteNadineGroupAction(n);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
...
function NewProjectDialog({
  ...
  const [category, setCategory] = React.useState<NadineItemCategory>("otros");
  const [group, setGroup] = React.useState<string>(defaultGroup ?? "");

  const createMutation = useMutation({
    mutationFn: async () => {
      const result = await createNadineProjectAction({
        name: name.trim(),
        group: group || null,
        title: title.trim(),
        category,
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={category}
              onChange={(e) =>
                setCategory(e.target.value as NadineItemCategory)
              }
