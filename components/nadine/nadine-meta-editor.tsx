...
import { updateNadineMetaAction } from "@/app/actions/nadine";
...
export function NadineMetaEditor({
  project,
  color,
  emoji,
  trigger,
  ...
  const mutation = useMutation({
    mutationFn: async (patch: {
      ...
      const result = await updateNadineMetaAction({
        project,
        ...patch,
      });
            ...
            {NADINE_COLORS.map((c) => (
              <button
                ...
                className={cn(
                  ...
                  NADINE_COLOR_CLASS[c].chip,
                  color === c && "ring-2 ring-offset-1",
                  color === c && NADINE_COLOR_CLASS[c].ring,
                )}
                aria-label={c}
              >
