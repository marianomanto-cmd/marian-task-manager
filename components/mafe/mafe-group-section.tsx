...
import {
  archiveMafeProjectAction,
  deleteMafeProjectAction,
  renameMafeProjectAction,
  updateMafeMetaAction,
} from "@/app/actions/mafe";
import { MafeAvatar } from "@/components/mafe/mafe-avatar";
import { MafeItemRow } from "@/components/mafe/mafe-item-row";
import { MafeMetaEditor } from "@/components/mafe/mafe-meta-editor";
import { MafeQuickAddRow } from "@/components/mafe/mafe-quick-add-row";
...
const STATUS_ORDER: MafeItemStatus[] = [
  "pending",
  "ongoing",
  "waiting",
  "done",
...
function ProjectName({
  ...
  const renameMutation = useMutation({
    mutationFn: async (to: string) => {
      const result = await renameMafeProjectAction({ from: project, to });
...
function GroupMenu({
  ...
  const groupMutation = useMutation({
    mutationFn: async (next: string | null) => {
      const result = await updateMafeMetaAction({ project, group: next });
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const result = await deleteMafeProjectAction(project);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
...
type GroupProject = {
  project: string;
  meta: MafeProjectMeta | undefined;
  items: MafeItem[];
};
...
function ProjectBlock({
  ...
  const qc = useQueryClient();
  const pm = meta ?? defaultMafeMeta(project);

  const stats = React.useMemo(() => {
    const counts: Record<MafeItemStatus, number> = {
      pending: 0,
      ongoing: 0,
      waiting: 0,
      done: 0,
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      const result = await archiveMafeProjectAction({
        project,
        archive: !inArchive,
      });
      if (!result.ok) throw new Error(result.message);
  ...
  const palette = MAFE_COLOR_CLASS[pm.color];
    ...
    <div>
            ...
            {STATUS_ORDER.map((s) =>
              stats[s] > 0 ? (
                <span
                  ...
                  title={MAFE_ITEM_STATUS_LABEL[s]}
                >
                  <span
                    className={cn(
                      "size-1.5 rounded-full",
                      MAFE_ITEM_STATUS_DOT[s],
                    )}
                  />
                  {stats[s]}
                </span>
      ...
      {open ? (
        <div className="animate-in fade-in-0 slide-in-from-top-1 duration-150">
          ...
          {canEdit ? <MafeQuickAddRow project={project} /> : null}
        </div>
      ) : null}
    </div>
  );
...
export function MafeGroupSection({
  groupKey,
  group,
  projects,
  canEdit,
  ...
  const color = mafePickStableColor(group);
  ...
  return (
    <section className="bg-card overflow-hidden rounded-xl border shadow-sm">
      ...
      >
        ...
        <MafeAvatar project={group} color={color} emoji={null} size="md" />
