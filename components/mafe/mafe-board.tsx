...
import {
  createMafeGroupAction,
  createMafeProjectAction,
  deleteMafeGroupAction,
  reorderMafeItemsAction,
} from "@/app/actions/mafe";
import { MafeGroupSection } from "@/components/mafe/mafe-group-section";
import {
  MAFE_ITEMS_KEY,
  mafeBoardKey,
  useMafeBoard,
} from "@/components/mafe/use-mafe-board";
...
import {
  ...
  MAFE_ITEM_STATUSES,
  MAFE_ITEM_STATUS_LABEL,
  type MafeDensityMode,
  type MafeGroup,
  type MafeItem,
...
const STATUS_OPTIONS = MAFE_ITEM_STATUSES.map((s) => ({
  value: s,
  label: MAFE_ITEM_STATUS_LABEL[s],
}));
const CATEGORY_OPTIONS = MAFE_ITEM_CATEGORIES.map((c) => ({
  value: c,
  label: MAFE_ITEM_CATEGORY_LABEL[c],
}));
...
export function MafeBoard() {
  const canEdit = true;
  const [view, setView] = React.useState<View>("active");
  const query = useMafeBoard({ archiveMode: view });
  const qc = useQueryClient();

  const [statuses, setStatuses] = React.useState<MafeItemStatus[]>([
    ...MAFE_ITEM_STATUSES,
  ]);
  const [categories, setCategories] = React.useState<MafeItemCategory[]>([
    ...MAFE_ITEM_CATEGORIES,
  ]);
  const [selectedGroup, setSelectedGroup] = React.useState<string | null>(null);
  const [search, setSearch] = React.useState("");
  const [showFilters, setShowFilters] = React.useState(true);
  const density: MafeDensityMode = "comfortable";
  ...
  const metaByProject = React.useMemo(() => {
    const map = new Map<string, MafeProjectMeta>();
  ...
  const grouped = React.useMemo(() => {
    const map = new Map<string, MafeItem[]>();
  ...
  const groupSections = React.useMemo(() => {
    const byGroup = new Map<string, { project: string; items: MafeItem[] }[]>();
  ...
  const reorderItemsMutation = useMutation({
    mutationFn: async (vars: { project: string; ids: string[] }) => {
      const result = await reorderMafeItemsAction(vars);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: mafeBoardKey({ archiveMode: view }) }),
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `board-mafe-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  const filtersActive =
    ...
    statuses.length !== MAFE_ITEM_STATUSES.length ||
    categories.length !== MAFE_ITEM_CATEGORIES.length;
    ...
    <section className="mx-auto flex w-full max-w-[96rem] flex-col gap-4 px-3 py-4 md:px-6 md:py-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl font-semibold tracking-tight md:text-2xl">
            {inArchive ? "Archivo · Board - Mafe" : "Board - Mafe"}
          </h1>
      ...
      {showFilters ? (
        <div className="flex flex-col gap-2">
          <FilterChips<MafeItemStatus>
            label="Status"
            options={STATUS_OPTIONS}
            selected={statuses}
            onChange={setStatuses}
          />
          <FilterChips<MafeItemCategory>
            label="Categoría"
...
function ManageGroups({
  ...
  const createMutation = useMutation({
    mutationFn: async (n: string) => {
      const result = await createMafeGroupAction(n);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (n: string) => {
      const result = await deleteMafeGroupAction(n);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
...
function NewProjectDialog({
  ...
  const [category, setCategory] = React.useState<MafeItemCategory>("otros");
  const [group, setGroup] = React.useState<string>(defaultGroup ?? "");

  const createMutation = useMutation({
    mutationFn: async () => {
      const result = await createMafeProjectAction({
        name: name.trim(),
        group: group || null,
        title: title.trim(),
        category,
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={category}
              onChange={(e) =>
                setCategory(e.target.value as MafeItemCategory)
              }
