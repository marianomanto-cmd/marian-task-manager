...
import {
  archiveMafeItemAction,
  deleteMafeItemAction,
  updateMafeItemAction,
...
export function MafeItemRow({
  ...
  const updateMutation = useMutation({
    mutationFn: async (patch: Partial<MafeItem>) => {
      const result = await updateMafeItemAction({ id: item.id, ...patch });
  ...
  const archiveMutation = useMutation({
    mutationFn: async (archive: boolean) => {
      const result = await archiveMafeItemAction(item.id, archive);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: MAFE_ITEMS_KEY }),
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const result = await deleteMafeItemAction(item.id);
      if (!result.ok) throw new Error(result.message);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: MAFE_ITEMS_KEY }),
    onError: (err: Error) => showToast({ title: err.message }),
    ...
    >
      <span
        ...
        className={cn(
          ...
          MAFE_ITEM_STATUS_BAR[item.status],
        )}
      />
...
function StatusCell({
  ...
  value: MafeItemStatus;
  canEdit: boolean;
  onChange: (next: MafeItemStatus) => void;
}) {
  const chip = (
    <span
      className={cn(
        "inline-flex h-6 items-center gap-1.5 rounded-full border px-2 text-[11px] font-medium",
        MAFE_ITEM_STATUS_CLASS[value],
      )}
    >
      <span
        className={cn("size-1.5 rounded-full", MAFE_ITEM_STATUS_DOT[value])}
      />
      {MAFE_ITEM_STATUS_LABEL[value]}
    </span>
  );
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start">
        {MAFE_ITEM_STATUSES.map((s) => (
          <DropdownMenuItem key={s} onClick={() => onChange(s)}>
            <span
              className={cn("mr-2 size-2 rounded-full", MAFE_ITEM_STATUS_DOT[s])}
            />
            {MAFE_ITEM_STATUS_LABEL[s]}
