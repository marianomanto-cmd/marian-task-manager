...
import {
  archiveYissItemAction,
  deleteYissItemAction,
  updateYissItemAction,
} from "@/app/actions/yiss";
import { YISS_ITEMS_KEY } from "@/components/yiss/use-yiss-board";
...
import {
  ...
  YISS_ITEM_CATEGORY_DOT,
  YISS_ITEM_CATEGORY_LABEL,
  YISS_ITEM_STATUSES,
  YISS_ITEM_STATUS_BAR,
  YISS_ITEM_STATUS_CLASS,
  YISS_ITEM_STATUS_DOT,
...
type RowProps = {
  item: YissItem;
  density: YissDensityMode;
  canEdit: boolean;
  draggable?: boolean;
};

export function YissItemRow({
  item,
  density,
  canEdit,
  draggable = true,
  ...
  const updateMutation = useMutation({
    mutationFn: async (patch: Partial<YissItem>) => {
      const result = await updateYissItemAction({ id: item.id, ...patch });
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: YISS_ITEMS_KEY }),
    onError: (err: Error) => showToast({ title: err.message }),
  });

  const archiveMutation = useMutation({
    mutationFn: async (archive: boolean) => {
      const result = await archiveYissItemAction(item.id, archive);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: YISS_ITEMS_KEY }),
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const result = await deleteYissItemAction(item.id);
      if (!result.ok) throw new Error(result.message);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: YISS_ITEMS_KEY }),
    onError: (err: Error) => showToast({ title: err.message }),
    ...
    >
      <span
        ...
        className={cn(
          ...
          YISS_ITEM_STATUS_BAR[item.status],
        )}
      />
...
function CategoryCell({
  ...
  value: YissItemCategory;
  canEdit: boolean;
  onChange: (next: YissItemCategory) => void;
}) {
  const content = (
    <>
      <span
        className={cn("size-2.5 rounded-full", YISS_ITEM_CATEGORY_DOT[value])}
      />
      <span className="text-muted-foreground text-xs md:hidden">
        {YISS_ITEM_CATEGORY_LABEL[value]}
      </span>
    </>
  );
  ...
  if (!canEdit)
    return (
      <span
        ...
        title={YISS_ITEM_CATEGORY_LABEL[value]}
      >
        {content}
      </span>
    );
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          className="flex h-8 items-center gap-1.5 md:h-6 md:justify-center"
          title={YISS_ITEM_CATEGORY_LABEL[value]}
          aria-label={`Categoría: ${YISS_ITEM_CATEGORY_LABEL[value]}`}
        >
          {content}
        </button>
      </DropdownMenuTrigger>
...
function StatusCell({
  ...
  value: YissItemStatus;
  canEdit: boolean;
  onChange: (next: YissItemStatus) => void;
}) {
  const chip = (
    <span
      className={cn(
        "inline-flex h-8 items-center gap-1.5 rounded-full border px-2.5 text-[11px] font-medium md:h-6 md:px-2",
        YISS_ITEM_STATUS_CLASS[value],
      )}
    >
      <span
        className={cn("size-1.5 rounded-full", YISS_ITEM_STATUS_DOT[value])}
      />
      {YISS_ITEM_STATUS_LABEL[value]}
    </span>
  );
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start">
        {YISS_ITEM_STATUSES.map((s) => (
          <DropdownMenuItem key={s} onClick={() => onChange(s)}>
            <span
              className={cn("mr-2 size-2 rounded-full", YISS_ITEM_STATUS_DOT[s])}
            />
            {YISS_ITEM_STATUS_LABEL[s]}
