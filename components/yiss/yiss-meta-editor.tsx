...
import { updateYissMetaAction } from "@/app/actions/yiss";
...
export function YissMetaEditor({
  project,
  color,
  emoji,
  trigger,
  ...
  const mutation = useMutation({
    mutationFn: async (patch: {
      ...
      const result = await updateYissMetaAction({
        project,
        ...patch,
      });
            ...
            {YISS_COLORS.map((c) => (
              <button
                ...
                className={cn(
                  ...
                  YISS_COLOR_CLASS[c].chip,
                  color === c && "ring-2 ring-offset-1",
                  color === c && YISS_COLOR_CLASS[c].ring,
                )}
                aria-label={c}
              >
