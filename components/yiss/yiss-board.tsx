...
import {
  createYissGroupAction,
  createYissProjectAction,
  deleteYissGroupAction,
  reorderYissItemsAction,
} from "@/app/actions/yiss";
import { YissGroupSection } from "@/components/yiss/yiss-group-section";
import {
  YISS_ITEMS_KEY,
  yissBoardKey,
  useYissBoard,
} from "@/components/yiss/use-yiss-board";
...
import {
  ...
  YISS_ITEM_CATEGORY_LABEL,
  YISS_ITEM_STATUSES,
  YISS_ITEM_STATUS_LABEL,
  type YissDensityMode,
  type YissGroup,
  type YissItem,
...
const STATUS_OPTIONS = YISS_ITEM_STATUSES.map((s) => ({
  value: s,
  label: YISS_ITEM_STATUS_LABEL[s],
}));
const CATEGORY_OPTIONS = YISS_ITEM_CATEGORIES.map((c) => ({
  value: c,
  label: YISS_ITEM_CATEGORY_LABEL[c],
}));
...
export function YissBoard() {
  const canEdit = true;
  const [view, setView] = React.useState<View>("active");
  const query = useYissBoard({ archiveMode: view });
  const qc = useQueryClient();

  const [statuses, setStatuses] = React.useState<YissItemStatus[]>([
    ...YISS_ITEM_STATUSES,
  ]);
  const [categories, setCategories] = React.useState<YissItemCategory[]>([
    ...YISS_ITEM_CATEGORIES,
  ]);
  const [selectedGroup, setSelectedGroup] = React.useState<string | null>(null);
  const [search, setSearch] = React.useState("");
  const [showFilters, setShowFilters] = React.useState(true);
  const density: YissDensityMode = "comfortable";
  ...
  const metaByProject = React.useMemo(() => {
    const map = new Map<string, YissProjectMeta>();
  ...
  const grouped = React.useMemo(() => {
    const map = new Map<string, YissItem[]>();
  ...
  const groupSections = React.useMemo(() => {
    const byGroup = new Map<string, { project: string; items: YissItem[] }[]>();
  ...
  const reorderItemsMutation = useMutation({
    mutationFn: async (vars: { project: string; ids: string[] }) => {
      const result = await reorderYissItemsAction(vars);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: yissBoardKey({ archiveMode: view }) }),
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `board-yiss-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  const filtersActive =
    ...
    statuses.length !== YISS_ITEM_STATUSES.length ||
    categories.length !== YISS_ITEM_CATEGORIES.length;
    ...
    <section className="mx-auto flex w-full max-w-[96rem] flex-col gap-4 px-3 py-4 md:px-6 md:py-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <h1 className="text-xl font-semibold tracking-tight md:text-2xl">
            {inArchive ? "Archivo · Board - Yiss" : "Board - Yiss"}
          </h1>
      ...
      {showFilters ? (
        <div className="flex flex-col gap-2">
          <FilterChips<YissItemStatus>
            label="Status"
            options={STATUS_OPTIONS}
            selected={statuses}
            onChange={setStatuses}
          />
          <FilterChips<YissItemCategory>
            label="Categoría"
...
function ManageGroups({
  ...
  const createMutation = useMutation({
    mutationFn: async (n: string) => {
      const result = await createYissGroupAction(n);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
      setName("");
      qc.invalidateQueries({ queryKey: YISS_ITEMS_KEY });
      onSelect(data.name);
    },
    onError: (err: Error) => showToast({ title: err.message }),
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (n: string) => {
      const result = await deleteYissGroupAction(n);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
...
function NewProjectDialog({
  ...
  const [category, setCategory] = React.useState<YissItemCategory>("otros");
  const [group, setGroup] = React.useState<string>(defaultGroup ?? "");

  const createMutation = useMutation({
    mutationFn: async () => {
      const result = await createYissProjectAction({
        name: name.trim(),
        group: group || null,
        title: title.trim(),
        category,
        ...
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={category}
              onChange={(e) =>
                setCategory(e.target.value as YissItemCategory)
              }
              className={cn(selectClass, "w-28")}
              aria-label="Categoría"
            >
              {YISS_ITEM_CATEGORIES.map((c) => (
                <option key={c} value={c}>
                  {YISS_ITEM_CATEGORY_LABEL[c]}
                </option>
              ))}
            </select>
          </div>
