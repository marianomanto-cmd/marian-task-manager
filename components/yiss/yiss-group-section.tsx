...
import {
  archiveYissProjectAction,
  deleteYissProjectAction,
  renameYissProjectAction,
  updateYissMetaAction,
} from "@/app/actions/yiss";
import { YissAvatar } from "@/components/yiss/yiss-avatar";
import { YissItemRow } from "@/components/yiss/yiss-item-row";
import { YissMetaEditor } from "@/components/yiss/yiss-meta-editor";
import { YissQuickAddRow } from "@/components/yiss/yiss-quick-add-row";
...
const STATUS_ORDER: YissItemStatus[] = [
  "pending",
  "ongoing",
  "waiting",
  "done",
...
function ProjectName({
  ...
  const renameMutation = useMutation({
    mutationFn: async (to: string) => {
      const result = await renameYissProjectAction({ from: project, to });
...
function GroupMenu({
  ...
  const groupMutation = useMutation({
    mutationFn: async (next: string | null) => {
      const result = await updateYissMetaAction({ project, group: next });
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const result = await deleteYissProjectAction(project);
      if (!result.ok) throw new Error(result.message);
      return result.data;
    },
    onSuccess: (data) => {
...
type GroupProject = {
  project: string;
  meta: YissProjectMeta | undefined;
  items: YissItem[];
};
...
function ProjectBlock({
  ...
  const qc = useQueryClient();
  const pm = meta ?? defaultYissMeta(project);

  const stats = React.useMemo(() => {
    const counts: Record<YissItemStatus, number> = {
      pending: 0,
      ongoing: 0,
      waiting: 0,
      done: 0,
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      const result = await archiveYissProjectAction({
        project,
        archive: !inArchive,
      });
      if (!result.ok) throw new Error(result.message);
  ...
  const palette = YISS_COLOR_CLASS[pm.color];
    ...
    <div>
            ...
            {STATUS_ORDER.map((s) =>
              stats[s] > 0 ? (
                <span
                  ...
                  title={YISS_ITEM_STATUS_LABEL[s]}
                >
                  <span
                    className={cn(
                      "size-1.5 rounded-full",
                      YISS_ITEM_STATUS_DOT[s],
                    )}
                  />
                  {stats[s]}
                </span>
      ...
      {open ? (
        <div className="animate-in fade-in-0 slide-in-from-top-1 duration-150">
          ...
          {canEdit ? <YissQuickAddRow project={project} /> : null}
        </div>
      ) : null}
    </div>
  );
...
export function YissGroupSection({
  groupKey,
  group,
  projects,
  canEdit,
  ...
  const color = yissPickStableColor(group);
  ...
  return (
    <section className="bg-card overflow-hidden rounded-xl border shadow-sm">
      ...
      >
        ...
        <YissAvatar project={group} color={color} emoji={null} size="md" />
