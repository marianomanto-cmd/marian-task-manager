...
const categorySchema = z.enum(
  MELY_ITEM_CATEGORIES as readonly [MelyItemCategory, ...MelyItemCategory[]],
);
const statusSchema = z.enum(
  MELY_ITEM_STATUSES as readonly [MelyItemStatus, ...MelyItemStatus[]],
);
const colorSchema = z.enum(
  MELY_COLORS as readonly [MelyColor, ...MelyColor[]],
);
const dateSchema = z
  .string()
...
export type ListMelyItemsInput = z.infer<typeof listSchema>;

/**
 * Anon Supabase client (no cookies, no session). Board - Mely is a public,
 * no-login board, so every request goes through the anon role regardless of
 * whether an agency member happens to be signed in. RLS on the mely_* tables
...
export type MelyBoardData = {
  items: MelyItem[];
  meta: MelyProjectMeta[];
  groups: MelyGroup[];
};

export async function listMelyBoardAction(
  input: ListMelyItemsInput = {},
): Promise<ActionResult<MelyBoardData>> {
  const parsed = listSchema.safeParse(input);
  if (!parsed.success) return asInvalid(parsed.error.message);

  try {
    const supabase = melyClient();
    let itemsQuery = supabase
      .from("mely_project_items")
      .select(ITEM_COLUMNS)
    ...
    return {
      ok: true,
      data: {
        items: (itemsRes.data ?? []) as MelyItem[],
        meta: (metaRes.data ?? []) as MelyProjectMeta[],
        groups: (groupsRes.data ?? []) as MelyGroup[],
      },
    };
  } catch (err) {
...
export async function createMelyItemAction(
  input: unknown,
): Promise<ActionResult<MelyItem>> {
  ...
  try {
    ...
    return { ok: true, data: data as MelyItem };
  } catch (err) {
    return asUnknown(err);
  }
}

export async function createMelyProjectAction(
  input: unknown,
): Promise<ActionResult<{ project: string; item: MelyItem }>> {
  ...
  try {
    ...
    const { error: metaError } = await supabase
      ...
      .insert({
        project: name,
        color: melyPickStableColor(name),
        emoji: null,
        grp: group,
        position,
      });
    ...
    return { ok: true, data: { project: name, item: item as MelyItem } };
  } catch (err) {
    return asUnknown(err);
  }
}

export async function updateMelyItemAction(
  input: unknown,
): Promise<ActionResult<MelyItem>> {
...
export async function archiveMelyItemAction(
  id: string,
  archive: boolean,
): Promise<ActionResult<MelyItem>> {
...
export async function deleteMelyItemAction(
  id: string,
...
export async function renameMelyProjectAction(
  input: unknown,
...
export async function deleteMelyProjectAction(
  project: string,
...
export async function archiveMelyProjectAction(
  input: unknown,
): Promise<ActionResult<{ project: string; count: number }>> {
...
export async function reorderMelyItemsAction(
  input: unknown,
...
export async function updateMelyMetaAction(
  input: unknown,
): Promise<ActionResult<MelyProjectMeta>> {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) throw new Error(error.message);
    return { ok: true, data: data as MelyProjectMeta };
  } catch (err) {
    return asUnknown(err);
  }
}

export async function createMelyGroupAction(
  input: unknown,
): Promise<ActionResult<MelyGroup>> {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) throw new Error(error.message);
    return { ok: true, data: data as MelyGroup };
  } catch (err) {
    return asUnknown(err);
  }
}

export async function deleteMelyGroupAction(
  input: unknown,
